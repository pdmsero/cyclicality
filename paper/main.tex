%% Migrated from vanilla article to shared house.cls in 2026-05-31 polish pass.
%% Original preamble preserved in archive/ if needed.

\documentclass{_latex/house}

%% Paper-specific packages not in shared preamble.
\usepackage{rotating}
\usepackage{dcolumn}
\usepackage{wrapfig}
\usepackage{endnotes}
...
\author[essex]{Pedro Ser\^odio\corref{cor1}}
\address[essex]{Department of Economics, University of Warwick}
\cortext[cor1]{Address: Department of Economics, University of Essex, CO4 3SQ Colchester, UK; \href{mailto:pdsero@essex.ac.uk}{pdsero@essex.ac.uk}. The author is grateful to Christian Ghiglino, Stefan Niemann, Katharine Rockett, Jo\~ao Santos Silva, George Symeonidis, Bruno Rocha, Gadi Barlevy and Paul Levine for advice and suggestions, and to seminar participants at the University of Essex and the University of Troms\o\ for helpful comments. Funded by Funda\c{c}\~ao para a Ci\^encia e a Tecnologia (FCT, Portugal).}
...
\begin{keyword}
Research \& Development \sep Business Cycles
\JEL E22 \sep E32 \sep O16 \sep O30 \sep O32
\end{keyword}

\end{frontmatter}

%% FCT funding logo, via \fctlogo from the shared preamble.
\fctlogo[5in]
...
\begin{figure}[hb]
 \caption{Cyclicality of R\&D at the aggregate level}
  \label{figure1}
 \centerline{ \includegraphics[width=\textwidth]{gdprnd.jpeg}}
\end{figure}
...
\begin{figure}[h!t]
 \caption{Growth rate of industry R\&D and Value of Shipments}
  \label{figure2}
 \centerline{ \includegraphics[width=\textwidth]{industry.jpeg}}
\end{figure}
...
\begin{align}
z|i_{i,t}={}&\beta_{0}+\beta_{1} \Delta \ln y_{i,t}+\sum_{k=1}^{K} \pi_{k} z|i_{i,t-k} \nonumber \\
&+\boldsymbol{\lambda X_{i,t}}+\boldsymbol{\gamma \tau_{t}}+\delta_{i}+\epsilon_{i,t} \label{eq:eq3}
\end{align}
...
Q_{i,j,t+1}= \left\{ \begin{array}{ll}
        ...
        Q_{i,j,t}, & \text{with probability} \quad \left(1-\mathcal{P}(Q_{i,j,t+1}=q_{i,j,m+1}|Q_{i,j,t}=q_{i,j,m}) \right).\end{array} \right.
$}
\label{equation5}
\end{equation}
...
\\
=& \left(q_{i,j}+(q_{i,j+1}-q_{i,j})\mathcal{P}\left(\frac{Z_{i,s}}{Q_{i,s}} \right) \right)\tilde{K}^{b}_{i,s+1} \\
&=\left(q_{i,j}+(q_{i,j+1}-q_{i,j})\mathcal{P}\left(\frac{Z_{i,s}}{Q_{i,s}} \right) \right)\tilde{K}^{e}_{i,s}
\end{align*}
...
\begin{figure}[h!t]
  ...
  \centering  
  \includegraphics[width=\textwidth]{typeofrnd.pdf} \\
\footnotesize
...
\begin{table}[h]\footnotesize
\caption{R\&D ($z_{i,j,t}$) and Sales ($y_{i,j,t}$) WW Index}
\begin{center}
{
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}
\begin{tabular}{l*{3}{c}}
...
\begin{align*}
KZ=&-1.001909\cdot CF+0.2826389 \cdot Q+3.139193 \cdot D \\
&-39.3678 \cdot Div-1.314759\cdot Cash
\end{align*}
...
\begin{align*}
WW=&-0.091\cdot CF-0.062\cdot DP+0.021\cdot D \\
&-0.044\cdot ln(A)+0.102\cdot Sale_i-0.035\cdot Sale
\end{align*}
