...
_spec = _ilu.spec_from_file_location("transform_stage2", Path(__file__).parent / "11_transform_stage2.py")
_mod  = _ilu.module_from_spec(_spec); _spec.loader.exec_module(_mod)
transform_stage2 = _mod.transform_stage2

ROOT = Path(__file__).resolve().parents[2]
DB_PATH = ROOT / "data" / "cyclicality.db"
REPORT_PATH = ROOT / "docs" / "reports" / "TRANSFORMATION_STAGE2_PARITY.md"
TOL = 1e-9
...
def main() -> int:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        stage1 = pd.read_sql_query("SELECT * FROM processed_alldata_stage1", conn)
        stage2 = pd.read_sql_query("SELECT * FROM processed_alldata_stage2", conn)

    recomputed, generated, skipped, assumptions = transform_stage2(stage1.copy())

    sort_cols = [c for c in ["key", "year", "gvkey", "datadate"] if c in recomputed.columns and c in stage2.columns]
    if sort_cols:
        recomputed = recomputed.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)
        stage2 = stage2.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)
    else:
        recomputed = recomputed.reset_index(drop=True)
        stage2 = stage2.reset_index(drop=True)

    row_match = len(recomputed) == len(stage2)

    common_generated = [c for c in generated if c in recomputed.columns and c in stage2.columns]
    missing_in_stage = [c for c in generated if c not in stage2.columns]

    per_var: list[tuple[str, int, float, float]] = []
    total_fail = 0
    for col in common_generated:
        fail, maxd, meand = numeric_diff(recomputed[col], stage2[col])
        per_var.append((col, fail, maxd, meand))
        total_fail += fail

    lines: list[str] = []
    lines.append("# Transformation Stage 2 Parity")
    lines.append("")
    lines.append("Parity check between recomputed Stage-2 transforms and stored `processed_alldata_stage2` table.")
    lines.append("")
    lines.append("## Summary")
    lines.append("")
    lines.append(f"- Row count recomputed: `{len(recomputed)}`")
    lines.append(f"- Row count stored: `{len(stage2)}`")
